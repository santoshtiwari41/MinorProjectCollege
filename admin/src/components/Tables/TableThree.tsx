...
import AddBatchPopup from '../batch/addBatch';
...
const TableThree: React.FC = () => {
  ...
  const handleBatch=(batchId:number)=>{
   navigate(`/tables/Students/${batchId}`)
  }
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {data.map((batchItem, index) => (
              <tr
                key={index}
                className="cursor-pointer hover:bg-gray-3 dark:hover:bg-meta-4"
